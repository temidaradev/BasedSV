...
//  Disable with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X]
//  Multiple Dear ImGui contexts support.
// Missing features or Issues:
//  [ ] Touch events are only correctly identified as Touch on Windows. This
//  create issues with some interactions. GLFW doesn't provide a way to identify
//  touch inputs from mouse inputs, we cannot call io.AddMouseSourceEvent() to
//  identify the source. We provide a Windows-specific workaround. [ ] Missing
//  ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress cursors.
...
ImGui_ImplGlfw_GetContentScaleForWindow(GLFWwindow *window);
IMGUI_IMPL_API float
ImGui_ImplGlfw_GetContentScaleForMonitor(GLFWmonitor *monitor);
